...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // dan juga memantau perubahan selanjutnya (login/logout).
    const { data: authListener } = supabase.auth.onAuthStateChange((_event, session) => {
        setSession(session);
    });

    // Berhenti memantau ketika komponen di-unmount untuk mencegah memory leak.
    return () => {
  ...
  // sementara sesi diperiksa di latar belakang. Ini adalah kunci untuk memperbaiki masalah layar kosong.
  return (
